    ...
    <section id="waitlist" className="py-32 bg-[#0D1B2A] overflow-hidden relative">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
        ...
        >
          ...
          <p className="text-lg md:text-2xl mb-12 text-white/50 font-light leading-relaxed" style={{ fontFamily: 'Nunito Sans, sans-serif' }}>
          ...
          <div className="flex flex-col sm:flex-row gap-6 justify-center">
            ...
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="w-full sm:w-auto">
              <Button asChild variant="outline" size="lg" className="h-16 md:h-20 w-full px-8 md:px-12 rounded-2xl border-2 border-white/10 bg-white/5 hover:bg-white hover:text-[#0D1B2A] text-white font-bold text-lg md:text-xl transition-all duration-300 backdrop-blur-xl cursor-pointer flex items-center justify-center gap-3">
                <a href={WHATSAPP_GROUP_URL} target="_blank" rel="noopener noreferrer">
                  Join Community <Users className="size-5 md:size-6" />
                </a>
              </Button>
            </motion.div>
          </div>
